...
import type { HeroImageDTO, SiteSettings } from "@/server/services/content";
import {
  ChevronLeft,
  ChevronRight,
  GoogleG,
...
export default function HeroSection({ images, settings }: Props) {
  return (
    <section className="relative w-full overflow-hidden bg-white">
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
        <div className="py-12 lg:w-1/2 lg:py-24 lg:pr-10">
          ...
          {/* Google rating */}
          <GoogleRatingCard rating={settings.googleRating} />
...
function GoogleRatingCard({ rating }: { rating: string }) {
  return (
...
function HeroCarousel({ images }: { images: HeroImageDTO[] }) {
  ...
  // Guard: an admin could deactivate every slide.
  if (count === 0) {
    return <div className="h-full w-full rounded-bl-[72px] bg-gray-100 lg:rounded-bl-[110px]" />;
  }
